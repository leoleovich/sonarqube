...
/**
 * Adds and removes user permissions. Both scopes global and project are supported.
 */
public class UserPermissionChanger {
  ...
  private void checkOtherAdminsExist(DbSession session, UserPermissionChange change) {
    if (SYSTEM_ADMIN.equals(change.getPermission()) && !change.getProjectId().isPresent()) {
      int remaining = dbClient.authorizationDao().countRemainingUsersWithGlobalPermissionIfExcludeUser(session,
